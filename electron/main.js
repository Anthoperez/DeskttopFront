// electron/main.js
const { app, BrowserWindow, shell, dialog } = require('electron'); // <-- AÑADIDO 'shell', quitado 'dialog' si no lo usas en otro lado
...
function createWindow() {
  ...
  mainWindow.webContents.session.on('will-download', (event, item, webContents) => {
    ...
    item.setSaveDialogOptions({
      ...
      defaultPath: path.join(downloadFolder, fileName),
      filters: [
    ...
    item.once('done', (event, state) => {
      if (state === 'completed') {
        ...
        console.log(` Descarga interrumpida o fallida: ${state}`);
        
        // Avisar al usuario si falló (probablemente archivo abierto)
        dialog.showErrorBox(
          'Error al Guardar', 
          `No se pudo guardar el archivo "${fileName}".\n\nPosible causa: El archivo está abierto en Excel o no tienes permisos.\n\nIntenta cerrarlo o guárdalo con otro nombre.`
        );
      } else {
